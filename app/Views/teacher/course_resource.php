            ...
            <div class="col-11 col-sm-11 col-md-11 col-lg-11 col-xl-11 p-0 text-center">
                <div class="card px-0">
                    <form id="msform" role="form" method="post" action="" enctype="multipart/form-data">
                        <fieldset>
                            <div class="form-card">
                            <h2 class="mt-4 step-titles">Course Resource Add</h2>
                           <div class="row">
                            <div class="col-md-6">
                                <div class="course-detil">
                                <label class="label-title">Resource Title</label>
                                ...
                                <div class="course-detil">
                                <label class="label-title">Resource File</label>
                                <input type="file" class="form-control" name="res_file" id="res_file" >   
                                </div>
...
<thead>
    <tr>
     ...
     <tbody>
       <?php $cnt=1; foreach ($resources as $r) {?>
       <tr id="row<?php echo $r['id']; ?>">
        <td><?php echo $cnt++; ?></td>
        <td><?php echo $r['title']; ?></td>
        <td><a href="<?php echo base_url($r['file']); ?>"><u>Download</u></a></td>
        <td id="status<?php echo $r['id']; ?>">
        <a href="javascript:;" class="status_change" data-status="<?php echo $r['status']=="y"?"n":"y"; ?>" data-id="<?php echo $r['id']; ?>"><span class="label label-<?php echo $r['status']=='y'?"success":"danger"; ?>"><?php echo $r['status']=='y'?"<i class='fa fa-check'></i> Shown":"<i class='fa fa-ban'></i>  Hidden"; ?></span>
        </a>
        </td>
        </tr>
        <?php } ?>
  ...
  $('#addBtn').click(function ()
   {
        ...
        var ext = myfile.split('.').pop();
            ...
            if(ext=="pdf" || ext=="docx" || ext=="doc" || myfile=="")
            {
                $('#msform').prop('action',base_path + 'course_resource');
                $('#msform').submit();
            }
            else
            {
     ...
     $('#example1').on('click','.status_change',function () {
            ...
            $.ajax({
                ...
                url : base_path + 'status_change_resource',
