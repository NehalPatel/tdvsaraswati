                ...
                <div class="col-md-12">
                    ...
                    <div class="card card-primary">
                        <div class="card-header">
                            <h3 class="card-title">Manage Qualities</h3>
                        </div>
                        <!-- /.card-header -->
                        <!-- form start -->
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label for="title">Select Quality</label>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                <?php $cnt=1; foreach ($quality as $a) {?>
                                    <tr id="row<?php echo $a['id']; ?>">
                                        <td><?php echo $cnt++; ?></td>
                                        <td><?php echo $a['quality']; ?></td>
                                        <td><a target="_blank" href="<?php echo base_url()."/".$a['video']; ?>">View Video</a> </td>
                                        <td>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
       ...
       $('#addBtn').click(function () {
            $('#form1').prop('action',base_path + 'teacher-insert-quality');
            $('#data_div').hide();
            $('#form_div').fadeIn();
       });
       ...
       $('#example1').on('click','.remove_data',function () {
          ...
          if(confirm('Are you sure to remove course??')){
              $.ajax({
                  type : 'post',
                  url : base_path + 'delete_quality/' + that.data('id'),
        ...
        $('#example1').on('click','.modify_data',function () {
            ...
            $.ajax({
                type : 'post',
                url : base_path + 'teacher-modify-quality/' + that.data('id'),
                success : function (response) {
                    //toastr.info(response);
                    var info=JSON.parse(response);
                    $('#hid').val(info.id);
                    $('#quality').val(info.quality);
                    $('#form1').prop('action',base_path + 'teacher-update-quality');
                    $('#data_div').hide();
                    $('#form_div').fadeIn();
                },
                error : function (response) {
